print("APP VERSION: 8", flush=True)
...
def webhook():
    ...
    if "callback_query" in update:
        ...
        if data.startswith("prod_"):
            ...
            if not info:
                send_message(chat_id, "Не удалось получить товар. Проверьте API ключи.")
                return "OK", 200
...
def show_products_page(chat_id, page):
    ...
    if not items:
        send_message(chat_id, "Не удалось получить список товаров. Убедитесь, что API ключи верны и есть товары.")
        return
    ...
    if len(items) == page_size:
        nav.append({"text": "Дальше →", "callback_data": f"page_{page + 1}"})
